(function($){
    $.extend($.fn.datagrid.defaults, {
        ...
        onAfterCellEdit: function(index, field, value){},
            ...
            if (input){
                if (accepted){
                    if (dg.datagrid('validateRow', cell.index)){
                        ...
                        var opts = dg.datagrid('options');
                        opts.onAfterCellEdit.call(target, cell.index, cell.field ,input.val());
                    } else {
                        dg.datagrid('gotoCell', cell);
                        input.focus();
                        return false;
    ...
    function enableCellEditing(target){
        var dg = $(target);
        if(dg.data("cellEditing"))
            return;
        dg.data("cellEditing",true);
        var opts = dg.datagrid('options');
        var panel = dg.datagrid('getPanel');
        panel.attr('tabindex',1).css('outline-style','none').unbind('.cellediting').bind('keydown.cellediting', function(e){
            var h = opts.navHandler[e.keyCode];
        ...
        panel.panel('panel').unbind('.cellediting').bind('keydown.cellediting', function(e){
            ...
            if (td.length){
                td.addClass('datagrid-row-over');
                $('tr.datagrid-row').removeClass('datagrid-row-over');
            }
        }).bind('mouseout.cellediting', function(e){
            var td = $(e.target).closest('td[field]');
            td.removeClass('datagrid-row-over');
    ...
    function isEmptyRow(target,params) {
        var dg = $(target);
        var rowData = dg.datagrid('getData').rows[params.index];
        var fields = dg.datagrid('getColumnFields');
        var flag = true;
        for(var name in rowData){
            if($.inArray(name, fields)>-1 && rowData[name] != "" && rowData[name] != null){
                flag = false;
                break;
            }
        }
    ...
    function getShowColumnFields(target){
        var dg = $(target);
        var fields = dg.datagrid("getColumnFields");
    ...
    $.extend($.fn.datagrid.methods, {
        ...
        isEmptyRow:function(jq,params){		// 判断该行数据是否为空 ，使用方法 $('#dg').datagrid('isEmptyRow',{index:index})
            return isEmptyRow(jq,params);
        },
        getShowColumnFields:function(jq){       //获取显示的行，使用方法 $('#dg').datagrid('getShowColumnFields')
            return getShowColumnFields(jq);
        },
        endCellEdit:function(jq){
            return endCellEdit(jq,true);
        }
    });
