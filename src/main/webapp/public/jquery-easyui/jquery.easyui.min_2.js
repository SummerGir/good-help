...
//xsb:改变表格高度
try{
if(set_table_height && typeof(set_table_height)=="function"){
set_table_height(opts.height,cpos.height);
}
}catch(e){
}
//end
...
//xsb：限制上下表格的最小高度
var v_top = e.pageY-$(_3b9).offset().top-_3c4.height()/2;
if(v_top < 148)
    v_top = 148;
if(v_top > 633)
    v_top = 633;
...
//xsb：限制表格上下最小高度
if(e.data.height){
var v_height = e.data.height;
if(v_height<148)
e.data.height = 148;
else if(v_height>633)
e.data.height = 633;
}
//end
...
window.treeRowIndex=_62e;//拿到当前tree的实例索引，用来获取td元素，一个页面只有一个tree时可用
