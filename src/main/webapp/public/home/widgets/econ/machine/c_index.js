 ...
 */
(function (itemId) {
    var color_b = "#43b5fe", color_z = "#0be054", color_n = "#9ad7fb", color_red = "#ff6238";
    var top_data = [];
    require(['echarts3'],function (eCharts) {
        //圆饼图
        var dom_b = document.getElementById("machine_container_b");
        ...
        //柱状图
        var dom_z = document.getElementById("machine_container_z");
        ...
        function load_echarts(param) {
            ...
            $.post("/public/homePageReport/getMoneyByKindStr.do",param,function (data) {
                ...
                if(data.data_b){
                    $(".machine_c_pom .mid-b").html((data.data_b.totle_check==0?0:data.data_b.totle_pay/data.data_b.totle_check*100).toFixed(0)+"%");
                ...
                if(data.yearData){
                    $(".machine_c_pom .child-check").text(parseFloat(Number(data.data_b.totle_check).toFixed(0)).toLocaleString());
                    $(".machine_c_pom .child-pay").text(parseFloat(Number(data.data_b.totle_pay).toFixed(0)).toLocaleString());
                }
                if(param.isFirst){
                    callback($.extend(true, param, {isFirst: false}), data.data_s, data.projectId);
                    $(".machine_c .project_name").text(data.projectName);
                }
            },"json")
        }
        function callback(param, dataArray, def) {
            $(".machine_c .icon-shezhi1").on("click", function () {
                if(typeof contextRight=='function') {
                    $(".modal.dialog_machine_project").modal();
                    loadProjectTree($("#machine_c_tree"), dataArray, def);
                    $(".modal.dialog_machine_project").modal("hide");
                    $(".modal.dialog_machine_project").modal();
                }else{
            ...
            $(".machine_c .btn-primary").on("click", function () {
                $(".modal.dialog_machine_project").modal("hide");
                ...
                $(".machine_c .project_name").text(projectNames.join(","));
                load_echarts(param);
            })
        }
        var project_tree;
            ...
            if(!project_tree) {
                project_tree = node.treegrid({
                    ...
                    onLoadSuccess: function (row, data) {
                        $(".dialog_machine_project").find(".tree-checkbox").on("click",function (e) {
                            e.stopPropagation();
