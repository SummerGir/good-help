...
(function (itemId) {
    ...
    require(['echarts3'],function (eCharts) {
        //圆饼图
        var dom_b = document.getElementById("machine_project_b");
        ...
        //柱状图
        var dom_z = document.getElementById("machine_project_z");
        ...
        function load_echarts(param) {
            ...
            $.post("/public/homePageReport/getMoneyByKindStr.do",param,function (data) {
                ...
                if(data.data_b){
                    $(".machine_p_pom .mid-b").html((data.data_b.totle_check==0?0:data.data_b.totle_pay/data.data_b.totle_check*100).toFixed(0)+"%");
                ...
                if(data.yearData){
                    $(".machine_p_pom .child-check").text(parseFloat(Number(data.yearData.caigou).toFixed(0)).toLocaleString());
                    $(".machine_p_pom .child-pay").text(parseFloat(Number(data.yearData.zhifu).toFixed(0)).toLocaleString());
                }
                if(param.isFirst){
                    callback($.extend(true, param, {isFirst: false}), data.data_s, data.projectId);
                    $(".machine_p .project_name").text(data.projectName);
                }
            },"json")
        }
        function callback(param, dataArray, def) {
            $(".machine_p .icon-shezhi1").on("click", function () {
                if(typeof contextRight=='function') {
                    $(".modal.dialog_machine_company").modal();
                    loadProjectTree($("#machine_p_tree"), dataArray, def);
                    $(".modal.dialog_machine_company").modal("hide");
                    $(".modal.dialog_machine_company").modal();
                }else{
            ...
            $(".machine_p .btn-primary").on("click", function () {
                $(".modal.dialog_machine_company").modal("hide");
                ...
                $(".machine_p .project_name").text(projectNames.join(","));
                load_echarts(param);
            })
        }
        var project_tree;
