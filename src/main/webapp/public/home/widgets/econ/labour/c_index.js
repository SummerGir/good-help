 ...
 */
(function (itemId) {
    var color_b = "#ffb320", color_z = "#ff3802", color_l = "#ff5a00", color_red = "#ff6238";
    var top_data = [];
    require(['echarts3'],function (eCharts) {
        //圆饼图
        var dom_b = document.getElementById("labour_container_b");
        ...
        //柱状图
        var dom_z = document.getElementById("labour_container_z");
        ...
        function load_echarts(param) {
            ...
            $.post("/public/homePageReport/getMoneyByKindStr.do",param,function (data) {
                ...
                if(data.data_b){
                    $(".labour_c_pom .mid-b").html((data.data_b.totle_check==0?0:data.data_b.totle_pay/data.data_b.totle_check*100).toFixed(0)+"%");
                ...
                if(data.yearData){
                    $(".labour_c_pom .child-check").text(parseFloat(Number(data.data_b.totle_check).toFixed(0)).toLocaleString());
                    $(".labour_c_pom .child-pay").text(parseFloat(Number(data.data_b.totle_pay).toFixed(0)).toLocaleString());
                }
                if(param.isFirst){
                    callback($.extend(true, param, {isFirst: false}), data.data_s, data.projectId);
                    $(".labour_c .project_name").text(data.projectName);
                }
            },"json")
        }
        function callback(param, dataArray, def) {
            $(".labour_c .icon-shezhi1").on("click", function () {
                if(typeof contextRight=='function') {
                    $(".modal.dialog_labour_project").modal();
                    loadProjectTree($("#labour_c_tree"), dataArray, def);
                    $(".modal.dialog_labour_project").modal("hide");
                    $(".modal.dialog_labour_project").modal();
                }else{
            ...
            $(".labour_c .btn-primary").on("click", function () {
                $(".modal.dialog_labour_project").modal("hide");
                ...
                $(".labour_c .project_name").text(projectNames.join(","));
                load_echarts(param);
            })
        }
        var project_tree;
            ...
            if(!project_tree) {
                project_tree = node.treegrid({
                    ...
                    onLoadSuccess: function (row, data) {
                        $(".dialog_labour_project").find(".tree-checkbox").on("click",function (e) {
                            e.stopPropagation();
