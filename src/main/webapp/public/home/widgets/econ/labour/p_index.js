...
(function (itemId) {
    ...
    require(['echarts3'],function (eCharts) {
        //圆饼图
        var dom_b = document.getElementById("labour_project_b");
        ...
        //柱状图
        var dom_z = document.getElementById("labour_project_z");
        var myChart_z = eCharts.init(dom_z, {width: '98%'});
        var option_z = {
            // color : [color_n, color_z, "#fff"],
            tooltip: {
                trigger: 'axis',
                axisPointer : {
                    type : 'shadow'
        ...
        function load_echarts(param) {
            ...
            $.post("/public/homePageReport/getMoneyByKindStr.do",param,function (data) {
                ...
                if(data.data_b){
                    $(".labour_p_pom .mid-b").html((data.data_b.totle_check==0?0:data.data_b.totle_pay/data.data_b.totle_check*100).toFixed(0)+"%");
                ...
                if(data.yearData){
                    $(".labour_p_pom .child-check").text(parseFloat(Number(data.yearData.caigou).toFixed(0)).toLocaleString());
                    $(".labour_p_pom .child-pay").text(parseFloat(Number(data.yearData.zhifu).toFixed(0)).toLocaleString());
                }
                if(param.isFirst){
                    callback($.extend(true, param, {isFirst: false}), data.data_s, data.projectId);
                    $(".labour_p .project_name").text(data.projectName);
                }
            },"json")
        }
        function callback(param, dataArray, def) {
            $(".labour_p .icon-shezhi1").on("click", function () {
                if(typeof contextRight=='function') {
                    $(".modal.dialog_labour_company").modal();
                    loadProjectTree($("#labour_p_tree"), dataArray, def);
                    $(".modal.dialog_labour_company").modal("hide");
                    $(".modal.dialog_labour_company").modal();
                }else {
            ...
            $(".labour_p .btn-primary").on("click", function () {
                $(".modal.dialog_labour_company").modal("hide");
                ...
                $(".labour_p .project_name").text(projectNames.join(","));
                load_echarts(param);
            })
        }
        var project_tree;
