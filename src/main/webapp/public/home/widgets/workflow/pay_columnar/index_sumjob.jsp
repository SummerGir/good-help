...
<div class="widget-charts">
    <%
        ...
        if(list==null || list.size()<=0){
            sb.append("<option value=''>--无权限--</option>");
        }else{
            for(Map<String,Object> m:list){
                sb.append("<option value='"+m.get("projectId").toString()+"'>"+m.get("projectName").toString()+"</option>");
            }
        }

        String content_url = "";
        AppTreeNode appTreeNode = CurrentApplicationTemplate.getAppTreeNodeByCode("factmoney"); //按资源代码获取资源
        if (appTreeNode != null) {
    ...
    <div class="widget-charts-panel" style="padding: 2px;">
        <div class="col-xs-8 col-md-8 col-md-offset-2 col-xs-offset-2" style="position:absolute; z-index: 1">
            <select  id="objectId_1" onchange="loadBar_1()" class="select2" style="width: 100%;" multiple>
                <%=sb.toString()%>
            </select>
        </div>
        <div id="pay_columnar_1"  style="max-width: 100%; min-height: 200px;"></div>
    </div>
    <script type="text/javascript">
        ...
        function loadBar_1(){
            var objectId = $("#objectId_1").val();
            var objectIds = "";
            if(objectId != null){
                objectIds = objectId.join(",");
            }
            $.getJSON("/public/home/sheet/plug_in_action.jsp",{action:"projectSumJob",ids:objectIds},function(res){
                require([
                            'echarts',
                            'echarts/chart/bar'
                        ],
                        function (ec) {
                            ...
                            var placeholder = $("#pay_columnar_1");
                            placeholder.unbind();
