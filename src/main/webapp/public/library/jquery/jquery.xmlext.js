...
; (function ($) {
    function mnpXml(opCode, xmlStr) {
        return this.each(function () {
            ...
            var node = xmlStr;
            if (jQuery.type(xmlStr) === "string") {
                node = $.parseXml(xmlStr).firstChild.cloneNode(true);
            }
            if (!jQuery.isXMLDoc(node)) return;
    ...
    $.fn.extend({
        ...
        createCDATASection: function (data) {
            var elem = this[0];
            try{
                return elem.createCDATASection(data);
            }catch(e){}
            return elem.ownerDocument.createCDATASection(data);
        },
        createComment: function (data) {
            var elem = this[0];
            try{
                return elem.createComment(data);
            }catch(e){}
            return elem.ownerDocument.createComment(data);
        },
        createProcessingInstruction: function (target, data) {
            var elem = this[0];
            try{
                return elem.createProcessingInstruction(target, data);
            }catch(e){}
            return elem.ownerDocument.createProcessingInstruction(target, data);
        },
        xml: function () {
            var elem = this[0];
