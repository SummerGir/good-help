            ...
            var uploadComplete = function (evt) {
                ...
                if ($.type(response) === "array") {
                    ...
                    self._document._update(fileObject.id,
                        response[0].uri,
                        response[0].size,response[0].contentType);
                    self._document._setStatus(fileObject.id, $.fn.document.status.VALID,response[0].contentType);
                } else {
                    ...
                    (function (fileObject) {
                        ...
                        image.onload=function(){
                            ...
                            EXIF.getData(this, function(){
                                time = EXIF.getTag(this, 'DateTimeOriginal');
                                ...
                                if(EIIS.browser.isIOS && orientation && orientation!=1 ){
                                    ...
                                    if(time!==undefined){
                                        ...
                                        con.fillText(time,(im.w-12*30+60),im.h-30);
                                    }else{
                                        if(!$.isEmptyObject(EXIF.getAllTags(this))){
                                            con.font="30px Arial";
                                            con.fillStyle = "#FF0000";
                                            var date=new Date();
                                            con.fillText(date.getFullYear()+"-"+(self.addZero(date.getMonth()+1))+"-"+self.addZero(date.getDate())+" "+self.addZero(date.getHours())+":"+self.addZero(date.getMinutes())+":"+self.addZero(date.getDate()),(im.w-12*30+60),im.h-30);
                                        }
                                    }
