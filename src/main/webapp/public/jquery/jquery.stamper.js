...
(function($) {
	$.fn.stamper = function(options) {
		var opts = $.extend({
			e_scale : 1, // 图片最终大小，在原始图片大小上的倍数
			b_scale : 5, // 图片初始大小，在原始图片大小上的倍数
			speed : 600, // 动画持续时间，单位毫秒
			left : 0, 	 // 最终成像 水平偏移量 px
			top : 0,	// 最终成像 垂直偏移量 px
		}, options);
		...
		this.each(function(index) {
			...
			img.src = opts.image;
			img.onload = function() {
				var orgiCoor = getImageCoordinate(target, img, opts.e_scale,opts.left,opts.top);
				var initCoor = getStartCoordinate(target, img, opts.b_scale);
	...
	function getImageElement(id) {
		var img = $("#" + id);
		if (img.length == 0) {
			return $("<img class='stamp_img' id=\"" + id + "\" />").appendTo($(document.body)).hide();
		} else {
			return img.hide();
		}
	}
	...
	function getImageCoordinate(target, image,multiple,left,top) {
