...
(function($) {
    ...
    var isphone=0;
    try{
        isphone=window.parent.parent.isphonee();
    }catch (e){
        try{
            isphone=window.parent.isphonee();
        }catch (ee){
        }
    }
    var isapple=0;
    try{
        isapple=window.parent.parent.isapple();
    }catch (e){
        try{
            isapple=window.parent.isapple();
        }catch (ee){
        }
    }
    var quanping='';
    if(isapple!=1){
        quanping='<span class="oper_fullscreen" title="查看全屏"><i class="icon_tool-fullscreen"></i></span>';
    }
    var maxState;
    try{
        maxState=window.parent.parent.maxState;
    }catch (e){
        maxState=false;
    }
//图片查看器
    $.fn.extend({
        ...
        photoGallery: function(options) {
            ...
            $prev.on('click',function(){
                ...
                toggleImage();
                $image[0].addEventListener("touchmove",touchmove,false);
                $image[0].addEventListener("touchstart",touchstart,false);
            ...
            $fullscreen.on(myClick, function(){
                var parentD = window.parent.document;
                var J = $(parentD.getElementById("J_pg"));
                if(!isMax){
                    isMax = true;
                    ...
                    if(maxState){
                        ...
                        window.parent.parent.imgBodyMove(true);
                    }else{
                        J.css({
                            top: 0,
                            left : 0,
                    ...
                    if(maxState){
                        window.parent.parent.imgBodyMove(false);
                    }
                }
            });

            //放大图片
            $bigger.on(myClick, function(){
                biggerImage();
            });

            //缩小图片
            $smaller.on(myClick, function(){
                smallerImage();
            });

            //旋转
            $rotate.on(myClick, function(){
            ...
            function showThumbnails(width, height){
                ...
                if(width > document.body.clientWidth || height > document.body.clientHeight){
                    var start=window.parent.parent.isphonee();
                    if(start==0){
                        $thumbnails.show();
                    }
                    setThumbnails();
                    try{
                        window.parent.parent.moveState=false;
                    }catch (e){}
                } else{
                    $thumbnails.hide();
                    try{
                        window.parent.parent.moveState=true;
                    }catch (e){}
                }
            }
    ...
    function changeHW(H,W){
        var hh=0;
        if(window.closeState()=="1"){
            if((H-W)>230){
                hh=W;
            }else{
                hh=H;
    ...
    $.extend({
        ...
        openPhotoGallery : function(obj){
            ...
            var img = $img[0],
                imgHeight = changeHW(img.naturalHeight,img.naturalWidth),
            ...
            if(isphone==1){
                if(imgHeight-imgWidth>200){
                    imgHeight=imgHeight-(imgHeight-imgWidth)-100;
                }
            }
            ...
            $gallerys.find(".gallery-pic").each(function(i, elem){
                var url = this.src,
                    img = $(this)[0],
                    nH = changeHW(img.naturalHeight,img.naturalWidth),
                    nW = img.naturalWidth,
                    ratio  = nW / nH,
                    w = nW,
                    h = nH;
            ...
            $("<iframe></iframe>").appendTo("body")
                .attr("id", "J_pg")
                .attr("src", "jquery-photo-gallery/gallery.jsp")
                .css({
                    position : "absolute",
            ...
            try{
                window.parent.parent.changeImgDivWidth(winWidth);
            }catch (e){
        ...
        initGallery : function(){
            ...
            $(".closeWin").click(function(){
                if(window.parent.parent.moveState!==undefined && window.parent.parent.nowMaxState!==undefined){
                    window.parent.parent.moveState=true;
                    window.parent.parent.nowMaxState=false;
                }
                window.parent.parent.closediv();
